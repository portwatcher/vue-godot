...
- `npm whoami` succeeds as `skyquakers` in the current environment. Real local publishing reaches npm, but the registry requires an OTP; `GODOT_BIN=/Users/jury/Developments/Godot/editor/macos-editor-4.4-v8 npm run release:publish -- --yes` passed release preflight and stopped on `EOTP` before publishing `@vue-godot/browser@0.0.1`. The GitHub `Publish` workflow also reached `npm publish`, but npm rejected OIDC publishing for `@vue-godot/browser@0.0.1` with `E404` / no permission, so npm trusted publishing is not attached for the package name yet. Npm's `trust` command can only configure packages that already exist on the registry, so it cannot attach trusted publishing for the unpublished `@vue-godot/browser` or `@vue-godot/html` package records. `npm trust list` for the existing packages also stops on `EOTP` and requires browser/2FA authentication in this environment.
...
- Configure npm trusted publishing for existing packages with `npx npm@latest trust github <package> --repository portwatcher/vue-godot --file publish.yml --allow-publish --yes` after completing npm browser/2FA authentication.
- Run `npm run release:publish -- --yes --otp <code>` locally for the first publish of package names that do not yet exist on npm, or configure npm trusted publishing/permissions so the GitHub `Publish` workflow can publish `@vue-godot/browser@0.0.1`.
- After `@vue-godot/browser` and `@vue-godot/html` exist, attach their trusted publishers and use the `Publish` workflow for future releases.
