...
test('platform evidence audit rejects placeholder metadata and skip reasons', () => {
  ...
  template.android.evidenceUrl = '<android-non-local-device-evidence-url>'
...
test('record-platform-evidence CLI rejects unreplaced placeholders', () => {
  ...
  try {
    ...
    assert.equal(blankMetadataResult.status, 1)
    assert.match(
      blankMetadataResult.stderr,
      /android\.deviceModel requires a non-empty value/,
    )
