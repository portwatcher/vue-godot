...
| `<Span>` | `html` | `partial` | `Label` | All Godot UI targets | None | Unit, html-demo | Text display subset with flattened slot text, style text/size/display/opacity props, registered/local font family loading, process-wide scaling of explicit numeric font sizes, and tooltip-backed labels/hints; not a DOM inline layout box. |
...
| `<Button>` | `html` | `partial` | `Button` | All Godot UI targets | None | Unit, html-demo | Click maps to Godot pressed signal and is suppressed while disabled; supports text slot extraction, style text/size/display props, `autoFocus`, focus traversal props, `minTouchTarget`, and tooltip-backed labels/hints. |
| `<Input>` | `html` | `partial` | `LineEdit`, `CheckBox`, `ButtonGroup`, `HSlider` | All Godot UI targets | None | Unit, html-demo | Supports text, password, checkbox, radio, and range subsets with `v-model`; text/password support placeholder, `readonly`, and `maxLength`; range supports `min`/`max`/`step`; disabled/readonly controls suppress model updates; supports style size/display/opacity props, text styling for LineEdit, `autoFocus`, focus traversal props, `minTouchTarget`, and tooltip-backed labels/hints. |
| `<Textarea>` | `html` | `partial` | `TextEdit` | All Godot UI targets | None | Unit, html-demo | Multiline text editing subset with `v-model`, placeholder, disabled/readonly, rows/cols sizing, style text/size/display/opacity props, `autoFocus`, focus traversal props, `minTouchTarget`, and tooltip-backed labels/hints; browser selection APIs are not implemented. |
| `<Select>` / `<Option>` | `html` | `partial` | `OptionButton` | All Godot UI targets | None | Unit, html-demo | Option model subset with `v-model`, `value`, disabled options, selected defaults, style text/size/display/opacity props, `autoFocus`, focus traversal props, `minTouchTarget`, and tooltip-backed labels/hints; not a native HTML select. |
| `<Img>` | `html` | `partial` | `TextureRect`, `ResourceLoader`, browser blob/data helpers | All Godot UI targets | Asset import/export paths must be valid | Unit, html-demo | Supports Godot paths, relative paths, data/blob/remote image sources, style sizing, `objectFit`, `display`, transforms, and `alt`/accessibility tooltip metadata; image decoding support follows Godot loader support for each format. |
...
| `<Svg>` | `html` | `partial` | `TextureRect`, SVG/image loaders | All Godot UI targets | Asset import/export paths must be valid | Unit, html-demo | Supports Godot paths, relative paths, data/blob/remote SVG sources, `scale` for non-local rasterisation, style sizing, `objectFit`, `display`, transforms, and `alt`/accessibility tooltip metadata. SVG rendering still depends on Godot image support and conversion path. |
| `<A>` | `html` | `partial` | `LinkButton` / Godot URI handling | All Godot UI targets | OS URI handling availability | Unit, html-demo | Opens through Godot link behavior; supports shared focus and touch target props; `href` falls back to tooltip hint text when enabled. No browser navigation context; `target` is accepted for template compatibility but ignored. |
