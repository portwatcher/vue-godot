...
| CSS-like theme/style APIs (`defineHtmlTheme`, `createHtmlStyleSheet`, `defaultStyles`) | `html` | `partial` | Shared style resolver plus existing Godot style helpers and per-control theme overrides | All HTML-like component apps | None | Unit, html-demo, native-app-demo | Supports opt-in presets, structured component defaults, root tokens, type/class/group selectors, state pseudo-classes, and limited `min/max-width`, `min/max-height`, and `orientation` media queries for HTML-like components. No scoped SFC CSS, Vite CSS collection, DOM CSSOM, or computed styles. |
