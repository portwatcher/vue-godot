...
| `<CameraView>` | `html` | `partial` | `CameraServer`, `CameraFeed`, `CameraTexture`, `TextureRect` | Platforms where Godot reports camera feeds | Camera export permissions and native plugin setup remain app responsibilities | Unit, html-demo | Previews a selected camera feed through `CameraTexture`; no permission prompt, snapshot/capture API, or plugin fallback is bundled. |
