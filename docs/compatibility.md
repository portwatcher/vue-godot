...
| `@vue-godot/device/media-devices` bridge | `device` | `supported` | JavaScript adapter bridge for native Android/iOS camera/microphone plugins | Android, iOS, desktop where a plugin exists | Android camera/audio permissions; iOS camera/microphone plist keys; plugin export setup | Unit, html-demo smoke | Normalizes native plugin streams/tracks to `MediaDevicesAdapter` and maps platform, permission, missing-plugin, and export setup failures to typed capability states. |
...
and lowercase/PascalCase registration. Screen-reader labels, descriptions,
roles, live regions, and accessibility-tree integration are not exposed on the
current GodotJS baseline. Focusable controls support mount-time `autoFocus` / `autofocus`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `<Div>` | `html` | `partial` | Godot containers and style wrappers | All Godot UI targets | None | Unit, html-demo | CSS flex/grid subset with margin, padding, color/texture backgrounds, border, radius, percent-size anchors, basic transforms, and opacity/transform/size transitions plus registered keyframe animations; not a DOM element. |
| `<Span>` | `html` | `partial` | `Label` | All Godot UI targets | None | Unit, html-demo | Text display subset with flattened slot text, style text/size/display/opacity props, registered/local font family loading, and process-wide scaling of explicit numeric font sizes; not a DOM inline layout box. |
| `setHtmlFontScale()` / `getHtmlFontScale()` | `html` | `supported` | Godot theme font-size overrides | All Godot UI targets | None | Unit, physical Android app | Process-wide scale is clamped to 0.5–2 and applies to explicit numeric `fontSize` values on subsequent renders. |
| `<ScrollView>` | `html` | `partial` | `ScrollContainer` plus inner `<Div>` content wrapper | All Godot UI targets | None | Unit, html-demo, physical Android app | Scrollbar behavior follows Godot `ScrollContainer`; the content wrapper fills the viewport on both axes and grows with scroll-axis content. |
...
| `<Label>` | `html` | `partial` | `Label` plus optional inner `<Div>` wrapper | All Godot UI targets | None | Unit, html-demo | Groups label text with slot content visually; browser `for` / `id` focus binding and screen-reader semantics are not implemented. |
...
| `<SafeAreaView>` | `html` | `partial` | `DisplayServer.get_display_safe_area()` plus `MarginContainer` | Android cutouts where Godot reports safe area; desktop/editor usually zero insets | None | Unit, html-demo, physical Android app | Physical-window safe-area metrics are scaled into the configured project viewport; `fallbackInsets` can provide deterministic padding where metrics are unavailable. |
...
| `<Switch>` | `html` | `partial` | `CheckButton` | All Godot UI targets | None | Unit, html-demo | Binary toggle subset; supports `autoFocus`, focus traversal props, and `minTouchTarget`. |
| `<Button>` | `html` | `partial` | `Button` | All Godot UI targets | None | Unit, html-demo | Click maps to Godot pressed signal and is suppressed while disabled; supports text slot extraction, style text/size/display props, `autoFocus`, focus traversal props, and `minTouchTarget`. |
| `<Input>` | `html` | `partial` | `LineEdit`, `CheckBox`, `ButtonGroup`, `HSlider` | All Godot UI targets | None | Unit, html-demo | Supports text, password, checkbox, radio, and range subsets with `v-model`; text/password support placeholder, `readonly`, and `maxLength`; range supports `min`/`max`/`step`; disabled/readonly controls suppress model updates; supports style size/display/opacity props, text styling for LineEdit, `autoFocus`, focus traversal props, and `minTouchTarget`. |
| `<Textarea>` | `html` | `partial` | `TextEdit` | All Godot UI targets | None | Unit, html-demo | Multiline text editing subset with `v-model`, placeholder, disabled/readonly, rows/cols sizing, style text/size/display/opacity props, `autoFocus`, focus traversal props, and `minTouchTarget`; browser selection APIs are not implemented. |
| `<Select>` / `<Option>` | `html` | `partial` | `OptionButton` | All Godot UI targets | None | Unit, html-demo | Option model subset with `v-model`, `value`, disabled options, selected defaults, style text/size/display/opacity props, `autoFocus`, focus traversal props, and `minTouchTarget`; not a native HTML select. |
| `<Img>` | `html` | `partial` | `TextureRect`, `ResourceLoader`, browser blob/data helpers | All Godot UI targets | Asset import/export paths must be valid | Unit, html-demo | Supports Godot paths, relative paths, data/blob/remote image sources, style sizing, `objectFit`, `display`, and transforms; image decoding support follows Godot loader support for each format. |
...
| `<Svg>` | `html` | `partial` | `TextureRect`, SVG/image loaders | All Godot UI targets | Asset import/export paths must be valid | Unit, html-demo | Supports Godot paths, relative paths, data/blob/remote SVG sources, `scale` for non-local rasterisation, style sizing, `objectFit`, `display`, and transforms. SVG rendering still depends on Godot image support and conversion path. |
| `<A>` | `html` | `partial` | `LinkButton` / Godot URI handling | All Godot UI targets | OS URI handling availability | Unit, html-demo | Opens through Godot link behavior and supports shared focus and touch target props. No browser navigation context; `target` is accepted for template compatibility but ignored. |
| `<Audio>` | `html` | `partial` | `AudioStreamPlayer` | Platforms with audio output | Audio asset import/export paths | Unit, html-demo | Playback-only declarative subset with `src`, `autoplay`, `loop`, `muted`, `volume`, and `@ended`; no browser media element methods, text tracks, capture, or recording API. |
...
| `<Video>` | `html` | `partial` | `VideoStreamPlayer` | Platforms/codecs supported by Godot | Video asset import/export paths | Unit, html-demo | Playback-only declarative subset with `src`, `autoplay`, `loop`, `muted`, `volume`, style sizing, `opacity`, and `@ended`; codec/platform support follows Godot and no browser media element methods, text tracks, capture, or recording API are implemented. |
...
| CSS-like theme/style APIs (`defineHtmlTheme`, `createHtmlStyleSheet`, `defaultStyles`, `@vue-godot/html/vite`) | `html` | `partial` | Shared style resolver plus existing Godot style helpers, per-control theme overrides, and Vite global CSS import collection | All HTML-like component apps | None | Unit, html-demo, native-app-demo | Supports opt-in presets, structured component defaults, root tokens, type/class/group selectors, state pseudo-classes, limited `min/max-width`, `min/max-height`, and `orientation` media queries, and global `.css` imports through the Vite plugin. No scoped SFC CSS, DOM CSSOM, or computed styles. |
...
| `vue-godot doctor` | `cli` | `supported` | Local filesystem diagnostics | Local development, migration triage, and release review | Node.js, package installs, GodotJS typings, export presets, plugin-backed API setup, optional migration source scan | Unit | Static guardrail only; `--migration` reports small-change, medium, and rewrite areas for unsupported CSS, DOM assumptions, and browser tag replacements. Warnings must still be validated in the Godot editor and on target devices. |
