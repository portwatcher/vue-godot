...
| `vue-godot create` | `cli` | `supported` | Project templates | Local development | Node.js, GodotJS editor | CLI smoke, unit | Generated projects include production export guidance and a non-failing export-setting check backed by `vue-godot doctor --exports-only`; `--device` adds `@vue-godot/device`; production app profiles are still planned. |
