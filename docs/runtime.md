...
| Vue SFCs compiled by Vite                      | Supported                   | Templates compile to runtime-core render functions. The app bundle keeps `godot` external for the standalone runtime.                                              |
...
| CSS transitions and DOM transition hooks                            | DOM transition lifecycle is unsupported in runtime | Use Godot animation systems or explicit reactive prop updates. `@vue-godot/html` supports limited Tween-backed inline style transitions and registered keyframe animations for `opacity`, `transform`, `width`, and `height`. |
| Browser focus, accessibility tree, and ARIA semantics               | Screen-reader semantics are unsupported in runtime | Godot focus APIs remain available for keyboard/controller navigation. Native accessibility requires a future stock-Godot API and generated-binding upgrade.                                                                   |
