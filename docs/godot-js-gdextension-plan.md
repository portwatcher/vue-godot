...
| 2026-08-10 | 10    | Resolve the npm launcher by native platform, using `npm.cmd` on Windows and `npm` elsewhere. A clean Windows Server 2025 export reached the packaged matrix but Node's direct child-process lookup could not execute the POSIX launcher name. Enabling an ambient shell for every subprocess or skipping application rebuilds on Windows were rejected.                                                                                                                                                                                                                                              | Export-smoke application builds keep the same npm workspaces and arguments while executing without shell-dependent command lookup on native Windows.                                                                              | `ab41e2b` |
