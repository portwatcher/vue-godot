...
Status: implemented and supported on `develop`; all required migration and
...
- [x] `packages/godot-js-runtime` contains the native source, dependency lock,
      build tooling, public TypeScript helper API, installer, package metadata,
      license notices, and standalone README.
- [x] An official Godot 4.4 editor loads `godot_js_runtime.gdextension` without
      module patches, custom engine builds, or unresolved native dependencies.
- [x] The current stable official Godot editor does the same.
- [x] A `.js`, `.mjs`, or `.cjs` file exporting a Godot class can be attached to
      a scene and receives required lifecycle callbacks.
- [x] The virtual `godot` module exposes the classes, singletons, enums,
      constants, methods, properties, signals, and value types needed by the
      runtime contract and existing Vue Godot packages.
- [x] Variant conversion, object identity, reference ownership, garbage
      collection, call errors, and signal disconnection pass automated stress
      tests without leaks, stale pointers, or double frees.
- [x] Both ESM and CommonJS entry modules work, including the relative chunk
      loading produced by the repository's current Vite builds.
- [x] TypeScript users receive deterministic declarations generated from a
      stock Godot extension API dump, with no GodotJS-generated files required.
- [x] Source-mapped errors identify the original TypeScript or Vue source when
      a map is available, and ordinary JavaScript errors identify resource path,
      line, column, message, and stack.
- [x] Editor file changes reload scripts safely and the existing reload smoke
      behavior passes against stock Godot.
- [x] `apps/js-runtime-demo` proves standalone use and contains no Vue or
      `@vue-godot/*` dependency.
- [x] `apps/html-demo`, `apps/native-app-demo`, `apps/game-ui-demo`, and all
      smaller fixtures run through the GDExtension using official Godot.
- [x] The CLI can create and integrate projects, install the extension, generate
      types, diagnose setup, and prepare exports without mentioning a custom
      GodotJS editor.
- [x] Debug and release artifacts are built and smoke-tested for the required
      desktop, Android, iOS, and Web matrix defined below.
- [x] Release archives contain checksums, provenance, licenses, the
      `.gdextension` manifest, and only repository-produced binaries.
- [x] Root build, unit, integration, serious-example, performance, release
      preflight, and stock-Godot smoke gates pass.
- [x] The old GodotJS setup action, download script, tests, cached paths,
      generated binding templates, CI assumptions, and documentation are
      removed or replaced.
- [x] Open GitHub issues affected by the migration are closed or updated with
      commit hashes and remaining work, including tracker issues.
- [x] The implementation and documentation are pushed to `develop`, and the
...
| 10. Release hardening and issue audit             | Complete | `1d477fe`, `5434ba8`, `098717b`, `6313c13`, `6bf50ee`, `ab41e2b`, `a81a8c6`, `a1dab41` | From committed state, the complete in-tree and clean packaged matrices each pass 24 exports: 20 automated launches and four explicit unsigned iOS device/simulator link gates. Evidence hashes are `f8c1bde71a602252c8e262f4e5ad2204dab6bd49c76f28737c54f72ff2780103` and `d7334ac55f48e2119fe649ca12f73884917d97e2bc1e4fc4139d76dd5369d5dc`. All 14 targets/20 payloads pass architecture, symbols, shared-dependency, manifest, checksum, provenance, license/notice, and legacy-identity verification; two independent repackagings reproduce all six release archives and metadata byte-for-byte. The 16-package root build, all package suites, 61 root tests, CLI creation/integration, serious examples, eight deterministic budgets, live official-Godot performance, and the complete local release preflight pass. GitHub app and CLI audits find zero open issues or tracker checklists. The synchronized `develop` runs exposed clean-runner assumptions that the local matrices could not: `5434ba8` provisions Android tools explicitly, while `098717b` runs the pinned SCons wheel without `pip`/`venv`, selects the POSIX MinGW thread model, completes the binding header's standard-library includes, and supports current-Godot typed dictionaries and opaque callback pointers. Exact pinned Web, Linux GCC, and Windows MinGW container builds now pass debug/release; standalone smoke passes official Godot 4.7.1; 44 runtime tests and the complete official-Godot 4.4.1 root check pass with the canonical release-manifest checksum preserved. The committed release preflight passes. A clean Check run then exposed concurrent dependency-package rebuilds; `6313c13` makes Turbo finish dependency tests before dependents import their generated output, and a forced run from empty browser/device `dist` directories passes all package suites plus 62 root tests. The first hosted release matrix passed all six source builds and verified release packaging before Ubuntu 24.04's time64 audio-package transition blocked Linux setup and positional PowerShell archive arguments arrived empty on Windows. `6bf50ee` selects the available ALSA package and shares environment-backed zip extraction across editor and template setup; Ubuntu 22.04/24.04 probes, 63 script tests, a clean eight-export Apple matrix, and the complete root check pass. The next hosted run passed Check, both stock-Godot smoke versions, all six release builds, deterministic packaging/offline install, Android structural exports, and all macOS/iOS exports. Its remaining clean-runner failures identified three export portability assumptions: Linux binaries inherited Ubuntu 24.04's GLIBC 2.38, Windows Node could not resolve the POSIX `npm` launcher name, and Web resource discovery omitted raw JavaScript from the PCK. `ab41e2b` builds Linux on Ubuntu 22.04, selects the Windows npm batch launcher by native name, and explicitly includes the supported JavaScript module/resource formats in generated presets. Forty-five runtime tests, 63 script tests, a real packaged threaded-Web launch, and the complete official-Godot root check pass. That replacement hosted matrix passed Check, both stock-Godot smoke versions, sanitizer/native tests, all six builds, deterministic packaging/offline installation, and every Linux, Android, Web, macOS, and iOS export; Windows alone showed that Node 24 cannot spawn `npm.cmd` directly (`EINVAL`). `a81a8c6` now launches the batch file through the explicit Windows command processor with shell startup disabled and regression coverage. At the synchronized `develop` head `a1dab41`, hosted [Check run 31378028018](https://github.com/portwatcher/vue-godot/actions/runs/31378028018) (attempt 2 after one isolated upstream fetch failure), [stock-Godot run 31378028031](https://github.com/portwatcher/vue-godot/actions/runs/31378028031), [native/sanitizer run 31378028090](https://github.com/portwatcher/vue-godot/actions/runs/31378028090), and [release run 31378028035](https://github.com/portwatcher/vue-godot/actions/runs/31378028035) all pass. The final release run builds and verifies all six platform payloads, reproduces and clean-installs the archives, then passes Linux, Windows, Android, threaded Web, macOS, and unsigned iOS device/simulator export gates. A final GitHub audit again finds zero open issues or tracker checklists. |
...
| 2026-08-10 | 10    | Select `libasound2t64` when the runner's Debian package metadata exposes it and fall back to `libasound2` otherwise. Ubuntu 24.04 represents `libasound2` as a virtual package while Ubuntu 22.04 still provides the original concrete name; probes in both clean container releases install successfully. Pinning one distribution-specific name or relying on the changing hosted-runner image were rejected.                                                                                                                                                                                      | Official Godot setup retains the same runtime libraries while working on both the current Ubuntu 24.04 release runner and the older supported package layout.                                                                     | `6bf50ee` |
| 2026-08-10 | 10    | Pass Windows editor and export-template archive paths through scoped environment variables to one shared .NET zip extractor. Clean Windows Server 2025 showed that positional values following `powershell.exe -Command` were not bound to `$args`, and the `.tpz` template archive cannot rely on a `.zip`-only command. Environment-backed literal paths preserve spaces and metacharacters; duplicating extractors, interpolating paths into script text, or renaming downloaded archives were rejected.                                                                                          | Windows setup now extracts both official editor ZIPs and template TPZs through the same path-safe implementation; non-Windows `unzip` behavior and checksums are unchanged.                                                       | `6bf50ee` |
| 2026-08-10 | 10    | Build Linux release payloads on Ubuntu 22.04 and continue launching the packaged result in the digest-pinned Debian Bookworm container. Ubuntu 24.04 linked the otherwise valid extension against GLIBC 2.38, which the clean consumer container intentionally does not provide. Raising the consumer baseline, launching only on the build runner, or treating dependency-name inspection as sufficient were rejected.                                                                                                                                                                              | Linux x86_64 releases no longer inherit the newest hosted-runner glibc and remain launch-tested after deterministic packaging in an independent supported userspace.                                                              | `ab41e2b` |
| 2026-08-10 | 10    | Launch the Windows npm batch file through the explicit `ComSpec` command processor with `/d /s /c`; use direct `npm` execution on other platforms. The preceding native-name fix changed the clean Windows Server 2025 failure from unresolved `npm` (`ENOENT`) to a direct batch-file spawn error (`EINVAL`) under Node 24. Enabling an ambient shell for every subprocess, skipping application rebuilds on Windows, or depending on undocumented child-process batch handling were rejected.                                                                                                      | Export-smoke application builds keep the same npm workspaces and arguments, while Windows uses its required batch-file execution boundary and all other commands remain shell-free.                                               | `a81a8c6` |
| 2026-08-10 | 10    | Explicitly include `.js`, `.mjs`, `.cjs`, `.json`, and JavaScript source maps under `dist/` in every generated export preset. A clean Web export loaded the GDExtension but its PCK omitted `dist/player.js` when editor resource discovery did not classify the raw module; the focused packaged Chrome launch passes once the supported module graph is an explicit export input. Relying on editor cache state, including the entire generated tree indiscriminately, or disabling the Web launch gate were rejected.                                                                             | Desktop, mobile, and Web exports now package the complete supported JavaScript module graph deterministically, including Vite chunks and JSON modules, while retaining existing exclusions.                                       | `ab41e2b` |
