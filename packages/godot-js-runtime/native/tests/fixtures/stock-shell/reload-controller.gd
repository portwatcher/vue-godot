...
func _ready() -> void:
	...
	if not require_condition(
		...
		get_tree().quit(1)
		return
	print("[godotjs] PHASE4_DEFERRED_RELOAD PASS")
	...
	if not require_condition(probe.state == 777, "compatible property state"):
		get_tree().quit(1)
		return
	print("[godotjs] PHASE4_SOFT_RELOAD PASS")
	...
	if not require_condition(probe.state == Vector2(3, 4), "incompatible state reset"):
		get_tree().quit(1)
		return
	print("[godotjs] PHASE4_INCOMPATIBLE_STATE PASS")
	...
	if not require_condition(probe.state == Vector2(3, 4), "hard reload state reset"):
		get_tree().quit(1)
		return
	print("[godotjs] PHASE4_HARD_RELOAD PASS")
	...
	if not require_condition(probe.state == 22, "in-memory incompatible state reset"):
		get_tree().quit(1)
		return
	print("[godotjs] PHASE4_IN_MEMORY_RELOAD PASS")
	...
	if not require_condition(
		...
		get_tree().quit(1)
		return
	print("[godotjs] PHASE4_PREDELETE PASS")
