...
`npm run release:readiness -- --allow-open` prints the remaining final-readiness blockers without failing while the production-readiness TODO is still open. The strict form, `npm run release:readiness`, is for the final removal commit: it checks open TODO boxes, current real-device evidence, `release/release-readiness-evidence.json` for a successful warning-free Release Preflight workflow run, and public warning wording before experimental/not-production-ready text is removed.
