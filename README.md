...
After pushing a release candidate, `npm run release:ci -- --commit <sha> --output release/ci-runs.json` queries GitHub Actions for completed successful `Check` and `Godot Smoke` workflow runs on that exact commit. If it reports that the commit was not found on GitHub, push the release-candidate commit first. If either workflow did not run automatically for a docs/evidence-only release commit, dispatch the `Check` and `Godot Smoke` workflows manually on that exact ref before collecting CI evidence. Pass the generated file to `npm run release:evidence -- --ci-evidence release/ci-runs.json` so the real-device evidence uses the verified run URLs. After the `Release Preflight` workflow passes, rerun `npm run release:ci -- --commit <sha> --include-release-preflight --output release/ci-runs.json` so final readiness evidence can also import the verified preflight run URL from the same CI evidence file.
