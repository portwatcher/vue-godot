...
After pushing a release candidate, `npm run release:ci -- --commit <sha>` queries GitHub Actions for completed successful `Check` and `Godot Smoke` workflow runs on that exact commit. Use the printed run URLs as the `--check-run-url` and `--godot-smoke-run-url` inputs for `npm run release:evidence`.
