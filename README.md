...
After final real-device, CI, and Release Preflight evidence files are committed, run `npm run release:readiness -- --summary-output /tmp/vue-godot-readiness.json` without `--allow-open`, then run `npm run release:finalize-readiness -- --summary /tmp/vue-godot-readiness.json`. The finalizer refuses `--allow-open` summaries, unexpected readiness blockers, missing evidence-backed final TODO proof status, package description warning markers, or a dirty worktree. It only checks the final TODO boxes and removes the public warning wording after strict evidence is ready; commit those edits and rerun `npm run release:readiness` as the final gate.
