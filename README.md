...
npm run check:serious-examples # verifies native/game example app coverage and docs wiring
...
`npm run release:preflight` validates the publish path before a package release.
It runs the local quality gate unless `--skip-check` is passed, checks package
metadata and generated dependency versions, verifies `npm pack --dry-run`
contents against package exports, compares local package versions with the npm
registry, runs the dependency audit, checks serious example app coverage, and
runs the Godot smoke suite. Use `npm run release:preflight -- --local` when you
