...
Vite compiles `vue/src/main.ts` into `dist/app.js` (CJS format, `godot` external), then rebuilds on every Vue/TypeScript change. Press **F5** in the GodotJS editor to run the scene. Godot loads `dist/app.js`, the `_ready()` method fires, and Vue renders its component tree into the Godot scene. On editor reload or scene exit, `_exit_tree()` unmounts the Vue app.
