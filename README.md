...
`npm run release:evidence` assembles `release/real-device-evidence.json` and optionally `release/release-readiness-evidence.json` after the real Android/iOS checks and GitHub Actions runs exist. It reads the Android/iOS platform evidence JSON, strips the worksheet fields before writing final evidence, records current package versions, verifies the supplied or CI-evidence-derived Check/Godot Smoke/Release Preflight run URLs against GitHub Actions metadata, imports the warning count from `--release-preflight-summary release/release-preflight-summary.json` when readiness evidence is requested, rejects local-only or skipped preflight summaries, then validates the generated evidence before writing it. Directly copied platform evidence is rejected by the final evidence validator.
